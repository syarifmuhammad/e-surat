...
function download_docx(id, nama) {
    loading.value.open()
    axios.get(`${url}/incoming-letters/surat-keputusan-pengangkatan/${id}/download/docx`, {
        responseType: 'blob',
    }).then(res => {
...
function give_reference_number(id) {
    loading.value.open()
    axios.post(`${url}/incoming-letters/surat-keputusan-pengangkatan/${id}/reference-number`, {
        _method: 'PUT',
    }).then(res => {
        loading.value.close()
        Swal.fire({
...
function upload_signed_file() {
    ...
    axios.post(`${url}/incoming-letters/surat-keputusan-pengangkatan/${letter_id.value}/upload-signed-file`, request).then(res => {
        loading.value.close()
        Swal.fire({
            title: 'Berhasil!',
...
function sign() {
    ...
    axios.put(`${url}/incoming-letters/surat-keputusan-pengangkatan/${letter_id.value}/sign`, payload).then(res => {
        loading.value.close()
        Swal.fire({
            title: 'Berhasil!',
...
<template>
    ...
    <div class="flex flex-col bg-white rounded-lg">
        <div class="px-8 py-5 min-w-full inline-block align-middle">
            ...
            <CustomTable ref="table" :thead="thead" :url="`${url}/incoming-letters/surat-keputusan-pengangkatan`"
                v-slot="item">
                        ...
                        <div class="z-10 hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-[15rem] bg-white shadow-md rounded-lg p-2 mt-2 divide-y divide-gray-200"
                            ...
                            <div class="py-2 first:pt-0 last:pb-0">
                                <RouterLink target="_blank"
                                    :to="{ name: 'preview_surat_keputusan_pengangkatan_internal', params: { id: item.id } }"
                                ...
                                <RouterLink v-if="item.can_approved"
                                    :to="{ name: 'approve_surat_keputusan_pengangkatan_internal', params: { id: item.id } }"
                                    class="text-primary-500 cursor-pointer flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm hover:bg-gray-100 focus:outline-none focus:bg-gray-100">
                                    <Icon class="text-lg" icon="fluent:signed-24-regular"></Icon>
                                    Setujui Surat
                                </RouterLink>
                                <RouterLink v-if="!item.can_upload_verified_file && item.can_signed"
                                    :to="{ name: 'tanda_tangan_surat_keputusan_pengangkatan_internal', params: { id: item.id } }"
                                    class="text-primary-500 cursor-pointer flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm hover:bg-gray-100 focus:outline-none focus:bg-gray-100">
                                    <Icon class="text-lg" icon="fluent:signed-24-regular"></Icon>
                                    Tanda Tangani Surat
                                </RouterLink>
